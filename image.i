...
// FIXME:
{}extern img_segmentation_get_edge;
/* DOCUMENT e = img_segmentation_get_edge(sgm[, i]);

     The result is
       e(1, ) = number of inner pixels
       e(2, ) = number of edge pixels with E
       e(3, ) = number of edge pixels with W
       e(4, ) = number of edge pixels with E
       e(5, ) = number of edge pixels with W
...
// FIXME:
{}extern img_build_horizontal_chains;
/* DOCUMENT lst = img_build_horizontal_chains(sgm, ...);

     This function search the image segmentation SGM for chains of
     ...
     img_chain_get_segments(lst, j) - returns a vector of indices of segments
     ...
     img_chain_get_xmin(lst, j)
     img_chain_get_xmax(lst, j)
     img_chain_get_ymin(lst, j)
     img_chain_get_ymax(lst, j) - returns the bounding box of the
         j-th chain of LST after distortion correction.

   KEYWORDS

     LMIN = Minimum lenght of chains, default is 4.
     LMAX = Maximum lenght of chains, default is infinite.
     SATOL = Size absolute tolerance.
     SRTOL = Size relative tolerance.
     DRMIN = Minimum horizontal relative distance between successive segments.
     DRMAX = Maximum horizontal relative distance between successive segments.
     AATOL = Alignement absolute tolerance.
